...
export default ({ contentModuleId }) => {
    ...
    const TimelineEventCenter = (props) => {
        return (
            <Avatar className={props.color === "secondary" ? classes.secondary : classes.primary}>
                {props.children}
            </Avatar>
        );
    };
    ...
    const TimelineEvent = (props) => {
        if (props.center === "|") {
            ...
            return (
                <div>
                    <Grid container direction="row" justify="center" alignItems="center" wrap="nowrap" spacing={1}>
                        <Grid item xs={4}>
                            <TimelineEventLeft color={props.color}>{props.left}</TimelineEventLeft>
                        </Grid>
                        <Grid item>
                        ...
                        <Grid item xs={4}>
                            <TimelineEventRight color={props.color}>{props.right}</TimelineEventRight>
                        </Grid>
                    </Grid>
